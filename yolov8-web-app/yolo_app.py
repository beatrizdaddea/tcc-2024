...
def predict_on_image(image_stream):
    image = cv2.imdecode(np.asarray(
        bytearray(image_stream.read()), dtype=np.uint8), cv2.IMREAD_COLOR)
...
def home():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            file.stream.seek(0)
            original_image = cv2.imdecode(np.asarray(
                bytearray(file.stream.read()), dtype=np.uint8), cv2.IMREAD_COLOR)
